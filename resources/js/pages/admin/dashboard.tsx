...
interface PageProps {
    ...
    revenue_by_month: Array<{ period: string; total: number }>;
    filters: { status?: string; employee_id?: string; date_from?: string; date_to?: string };
    employees: Employee[];
}

export default function AdminDashboard({ stats, appointments, revenue_by_month, filters, employees }: PageProps) {
        ...
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    <Card>
                        <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Total Citas</CardTitle></CardHeader>
                        <CardContent><p className="text-2xl font-bold">{stats.total_appointments}</p></CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Citas Hoy</CardTitle></CardHeader>
                        <CardContent><p className="text-2xl font-bold">{stats.today_appointments}</p></CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Ingresos Totales</CardTitle></CardHeader>
                        <CardContent><p className="text-2xl font-bold">${Number(stats.total_revenue).toFixed(2)}</p></CardContent>
                    </Card>
                    <Card>
